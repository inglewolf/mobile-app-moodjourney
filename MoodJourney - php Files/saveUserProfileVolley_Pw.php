...
if (isset($_POST['userid']) && isset($_POST['username']) && isset($_POST['email']) && isset($_POST['birthdate']) && isset($_POST['newpassword']) ) {
    ...
    $sqlCommand = "UPDATE users SET Username = '$username',Email = '$email', Birthdate = '$birthdate', Password = '$newpassword' WHERE UserID = '$userid'";

    //Connect to db and run sql query
    $result = mysqli_query($myConnection->myconn, "$sqlCommand");

    if($result) { //Echo status
